...
class Post(models.Model):
    ...
    #models.PROTECT : Category 객체가 삭제가될 때, 연결된 Post객체가 존재하면, 삭제되지 않도록 막아주는 기능
    category = models.ForeignKey(Category,on_delete=models.PROTECT)
    #User 모델클래스 외래키 설정
    #models.CASCADE : User 객체가 삭제될 때 , 연결된 Post 객체들도 같이 삭제되는 기능
    user = models.ForeignKey(User,on_delete=models.CASCADE)
    #제목
    title = models.CharField('제목', max_length=100)
    #글 내용
    #TextField : 글자수 제한이 없는 문자열 저장공간
    #blank, null : XXXField생성자의 공통 매개변수
    #blank : form태그에서 필수로 입력하지 않아도 되는 영역을 설정
    #null : 데이터베이스에 저장할 때 값이 없어도 저장되도록 설정
    content = models.TextField('내용', blank = True, null = True)
    #생성일
    #auto_now_add = True : 객체생성시 서버의 시간을 자동으로 입력
    pub_date = models.DateField('작성일', auto_now_add=True)
    def __str__(self):
        return self.title
    #Meta클래스 정의해 정렬순서를 지정
    class Meta:
        #정렬 순서 지정
...
class PostFile(models.Model):
    ...
    file = models.FileField('파일', upload_to='piles/%Y/%m/%d')
