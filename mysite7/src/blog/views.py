...
#제네릭뷰 : 장고에서 제공하는 여러가지 뷰 기능을 규현한 클래스모음
...
#글 작성 페이지
#XXXMixin : 뷰클래스는 데코레이터를 지정할 수 없기 때문에 login_required와 같은 기능을 뷰에 지정할려면 XXXMixin 클래스를 상속받으면 된다.
#단, 제네릭뷰를 먼저 상속받은 다음에 Mixin클래스를 상속을 받아야 정상적으로 기능이 동작함
#LoginRequiredMixin : login_required 데코레이터와 동일한 기능
from django.contrib.auth.mixins import LoginRequiredMixin
#FormView : 폼클래스를 바탕으로 사용자에게 입력을 받아 처리하는 뷰
class Posting(LoginRequiredMixin, FormView):
    ...
    #POST방식으로 요청이 들어오면 사용자의 입력을 바탕으로 폼클래스객체 생성후 유효한 값인지(is_valid)
    #확인한 뒤 True값이 반환되면 폼 객체를 저장하는 함수가 호출
    
    
    #is_vaild() 이후에 호출되는 함수를 오버라이딩해 사용자가 업로드한 이미지나 파일을 바탕으로 PostImage, PostFile객체를 생성
    def form_valid(self, form):
        #Form 객체를 Post 객체로 변환
        #why? : Post객체를 데이터베이스에 저장할 때 user변수에 값이 들어있지 않은 상태기 때문에 에러가 발생
        #p : 사용자입력을 바탕으로 category, title, content 변수가 채워져있는 새로운 Post객체
        p = form.save(commit=False)
        #user 정보를 클라이언트의 유저정보로 대입
        #self.request : 해당 뷰를 요청한 클라이언트의 요청정보가 저장된 변수
        #self.request.user : 요청한 클라이언트의 User모델클래스 객체 저장변수
        ...
        for f in self.request.FILES.getlist('files'):
            pf = PostFile #새로운 PostFile객체 생성 - 데이터베이스에 저장되지 않음
            pf.post = p #새로 만들어진 Post 객체와 연동
            pf.file = f #사용자가 업로드한 파일은 FileFiled에 저장
            pf.save()
        #사용자가 업로드한 이미지데이터를 바탕으로 PostImage 객체 생성
        ...
        for i in self.request.FILES.getlist('images'):
            ...
            pi.save()
        #blog:detail 로 리다이렉트
        #새로 만들어진 Post 객체의 id값으로 detail뷰의 주소 전달
...
def post_delete(request, p_id):
    ...
    #추출한 객체의 user정보와 요청한 클라이언트의 user 정보를 비교
