...
해당하는 뷰클래스/함수를 호출하기 위한 등록 파일

뷰클래스/함수를 등록할때는 urlpatterns 변수의 요소로 추가하면 됨
요소로 추가할때는 path함수를 사용
...
#path(시작문자열, include(urls.py가 있는 위치)->시작문자열로 요청하는 모든 인터넷 주소는 include안에 있는 urls.py가 처리할 수 있도록 등록
#ex) path('tset/', include('vote.urls')) -> 127.0.0.1:8000/test/로 시작하는 모든 요청들을 vote폴더에 urls.py에서 처리할 수 있도록 등록
from vote.views import c_delete, c_update, c_regist, test, test_value, test_input, main, detail, vote, result, q_registe, q_update, q_delete
urlpatterns = [
    ...
    #127.0.0.1:8000/vote/qd/삭제할 Question객체의 id값/
    path('vote/qd/<int:q_id>/',q_delete, name = "qd" ),
    path('vote/cr/',c_regist, name = "cr" ),
    ...
    #127.0.0.1:8000/auth/로 시작하는 요청들을 social_django 어프리케이션의 urls.py에서 처리
    #include함수의 namespace 매개변수 : 해당 urls.py에 지정된 app_name값을 사용하지 않고 ㅐ로운 그룹명을 지정
